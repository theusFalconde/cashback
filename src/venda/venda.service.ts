...
export class VendaService {
  constructor(
    @InjectModel('Venda') private vendaModel: Model<Venda>,
  ...
  async update(id: string, vendaDto: VendaDto) {
    if (await this.verificarVendaUpdate(id)) {
      ...
      let usuario = await this.verificarUsuario(vendaDto.cpf);
      vendaDto.usuario = usuario;
      let vendaAtualizada = this.vendaModel.findByIdAndUpdate(id, vendaDto, {
        new: true,
      });
  ...
  async findCashbackAcumuladoByCpf(cpf) {
    ...
    let response = await this.httpService
      .get(VendaUtil.urlApiExterna + cpf, { headers: headerRequest })
      .toPromise();
    let valor = response.data.body.credit;
    return {
      codigo: Math.trunc(Math.random() * (99 - 1) + 1),
      valor: valor,
      data: new Date(),
  ...
  async findById(id): Model<Venda> {
    return await this.vendaModel
      .findById(id)
      .populate('usuario', this.usuarioModel)
      .exec();
  }
  ...
  modelToResponse(vendas) {
    return vendas.map(
      v =>
        new VendaResponse(v.id, v.codigo, v.valor, v.data, v.status, v.usuario),
    );
  }
  ...
  async verificarVendaUpdate(id) {
    ...
    if (!venda) {
      ...
      throw new BadRequestException(
        'Não é possível alterar o registro com o status "APROVADO"',
      );
    }
    return true;
  }
  ...
  async verificarVendaDelete(id) {
    ...
    if (!venda) {
      ...
      throw new NotAcceptableException(
        'Não é possível deletar um registro com status diferente de "Em validação"',
      );
    }
    return true;
  }
  ...
  async verificarCodigoVendaExiste(codigo) {
    ...
    if (venda) {
      throw new BadRequestException(
        `Já existe uma venda com o código ${codigo}`,
      );
    }
    return true;
  }
